...
for i=1:length(test)
    ...
    Y_60(i)=kNNClassifier(opt_test60(i,:),opt60,training(:,21),K);
    Y_60_N(i)=kNNClassifier(test_N_opt60(i,:),train_N_opt60,training(:,21),K);
    
    Y_70(i)=kNNClassifier(opt_test70(i,:),opt70,training(:,21),K);
    Y_70_N(i)=kNNClassifier(test_N_70(i,:),train_N_70,training(:,21),K);
    
    
    Y_80(i)=kNNClassifier(opt_test80(i,:),opt80,training(:,21),K);
    Y_80_N(i)=kNNClassifier(test_N_80(i,:),train_N_80,training(:,21),K);
    
    
    Y_90(i)=kNNClassifier(opt_test90(i,:),opt90,training(:,21),K);
    Y_90_N(i)=kNNClassifier(test_N_90(i,:),train_N_90,training(:,21),K);
end
